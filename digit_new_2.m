% Digital recognition
% In the second experiment, we demonstrate the effect of the dictionary size m on the recognition 805
performance.
...
Tdata=5;
iter=5;
dict_set=[100,300,500,700,900,1100,1300,1500];
tol_bcg=0.01;
itnlim=5;
for i=1:10
    group = group_set(i);
    ...
    for j=1:8
        dict_size=dict_set(j);
        
        b=min(dict_size,M);
        aim_rank=round(0.4*b);
        ...
        save(['exp3_2_1Result\', file],'train_t','accuracy','classify_t');
        fprintf('done!');
    ...
    % Parameters of RAKDL
    ktrain_params.itnlim=5;              % Maximum number of iterations for BCG to solve the least squares problem
    ktrain_params.tol_bcg=0.01;          % Convergence threshold for BCG
%     
%     
        ktrain_params.num_sub=0.2;
            ...
            file=strcat(num2str(i),num2str(Database),alg,'_',num2str(j),'.mat');
            save(['exp3_2_2Result\', file],'train_t','accuracy','classify_t');
            fprintf('done!');
        ...
        [train_map, test_map, virtual_train_t, virtual_test_t] = calc_virtual_map(train_data, test_data, ker_params,smp_type,c,c_rank);
           ...
           a=min(dict_size,size(train_map,1));
            ...
            file=strcat(num2str(i),num2str(Database),alg,'_',num2str(j),'.mat');
            save(['exp3_2_3Result\', file],'train_t','accuracy','classify_t','virtual_train_t','virtual_test_t');
            fprintf('done!');
