function [y,D,nz] = mydenoise(params,alg,msgdelta)
% RAODL algorithm added to the original denoise code
...
switch alg
    ...
    case 'KSVD'
      ...
      D = ksvd(params,verbose,msgdelta);
    case 'MOD'
      [D,~,~,~]=myMoD(params.data,params.Tdata,params.iternum,dictsize);
    case 'SGK'
      [D,~,~]=mysgk(params.data,dictsize,params.Tdata,params.iternum);
    case 'RAODL'
      a=min(dictsize,blocksize^2);
      t_rank=round(0.2*a);  %Ä¬ÈÏÊÇ0.2
      [U,S,V]=RAODL(params.data,params.Tdata,params.iternum,dictsize,t_rank,0.01,5);
      D=U*S*V';
    otherwise
      error('Unknown alg.');
end
